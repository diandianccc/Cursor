...
DROP POLICY IF EXISTS "Enable read access for all users" ON public.journey_maps;
DROP POLICY IF EXISTS "Enable update access for all users" ON public.journey_maps;

-- Create new policies for reading journey maps with comments
CREATE POLICY "Enable read access for all users" 
ON public.journey_maps FOR SELECT 
USING (true);

-- Create new policies for updating journey maps with comments
